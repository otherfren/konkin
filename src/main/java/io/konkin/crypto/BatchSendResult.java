...
public record BatchSendResult(
        Coin coin,
        String txId,
        Map<String, BigDecimal> amountsByAddress,
        BigDecimal totalFee,
    ...
    public BatchSendResult {
        ...
        if (amountsByAddress == null) throw new IllegalArgumentException("amountsByAddress must not be null");
        if (totalFee == null) throw new IllegalArgumentException("totalFee must not be null");
        extras = extras == null ? Map.of() : Map.copyOf(extras);
        amountsByAddress = Map.copyOf(amountsByAddress);
    }
}
