...
public final class BitcoinCashWalletFactory implements CoinWalletFactory {

    private static final Logger log = LoggerFactory.getLogger(BitcoinCashWalletFactory.class);
    ...
    public CoinWallet create(WalletConnectionConfig config) {
        return new BitcoinCashWallet(config);
    }
    ...
    public void prepareNode(WalletConnectionConfig config) {
        ...
        try {
            ...
            log.info("Loaded Bitcoin Cash wallet '{}' on node", walletName);
        } catch (JsonRpcStatusException e) {
            String msg = e.getMessage();
            if (msg != null && msg.contains("already loaded")) {
                log.debug("Bitcoin Cash wallet '{}' already loaded", walletName);
            } else {
                log.warn("Failed to load Bitcoin Cash wallet '{}': {}", walletName, msg);
            }
        } catch (Exception e) {
